...
Failure B  →  PixelLab any tool  →  Unity Filter Mode: Bilinear (default)  →  blurry
Failure C  →  PixelLab 32×32  →  correct scale + Point filter  →  no manual cleanup  →  low quality
Correct    →  PixelLab 32×32  →  nearest-neighbor scale to 16×16  →  manual cleanup in Aseprite  →  Point filter  →  ✓
```
 
The causal chain for Failure C (the most subtle, because it looks like the correct pipeline failed):
 
> Correctly generate at 32×32 in PixelLab → correctly scale to 16×16 with nearest-neighbor → skip manual cleanup in Aseprite → import to Unity → sprite reads as low quality or broken in scene context
 
The cleanup step is not optional polish. At 16×16, nearest-neighbor scaling makes compositional decisions the AI model did not optimize for at that density. Manual editing is the required final pipeline stage before import.
...
16×16 sits below the reliable working resolution for most PixelLab AI models. The correct pipeline for any 16×16 asset is: generate at 32×32 → scale down → manual cleanup. The sub-tool choice determines what you generate at 32×32.
 
| Sub-tool | Recommended size | Use for |
|---|---|---|
| Character Creator | **32×32** | Characters — scale to 16×16, clean up manually ✓ |
| Aseprite extension (Create Texture) | **32×32** | Tilesets and objects — same pipeline ✓ |
| Rotate Tool | **32×32** | Animation workflows |
| Skeleton Animation | **32×32** | Animation workflows |
 
**Why 32×32 over 24×24:** 32→16 is a 2:1 integer ratio — clean nearest-neighbor, predictable output, less manual correction. 24→16 is a 1.5:1 non-integer ratio — interpolation is unavoidable regardless of algorithm, more artifacts to fix.
