...
**P1.** A teammate generates a 32×32 tileable floor texture using PixelLab's Create Texture tool and imports it into Unity with Point filter mode and no compression. In the game, the tile seams are visible and the texture looks blurry along the edges. Identify which pipeline boundary is most likely responsible and explain what information would confirm your diagnosis.

**P2.** Using the bilinear interpolation formula from the Mechanism section, compute the output value for a 2×2 neighborhood with pixel values p₁=200, p₂=50, p₃=180, p₄=30 at fractional coordinates (u, v) = (0.5, 0.5). What does this value represent visually, and what would the nearest-neighbor output be for the same coordinates?

**P3.** You generate a 16×16 enemy sprite using PixelLab's BitForge canvas. The sprite looks correct in PixelLab's preview and in Unity's Scene view with Point filter and no compression. During playtesting, players report the sprite looks blurry during movement. Name at least one downstream processing step in Unity's rendering pipeline that could reintroduce interpolation artifacts at runtime.

**P4.** Describe a pipeline configuration in which using DALL-E to generate a 16×16 sprite would be the *correct* tool choice. What changes to the downstream pipeline are required to make the DALL-E output usable?

**P5.** The essay argues that "the prompt is the least important variable." Construct a counterargument: describe a scenario in which prompt design is, in fact, the highest-leverage intervention in an AI-assisted pixel art pipeline. What conditions must be true for this to hold?
